...
def _fetch_staking_hotkeys_parallel_pooled_v2(block_hash: str, block_number: int, 
                                            page_size: int = 1000, 
                                            fetch_workers: int = 12):
    """Version that preserves original key format but handles hashing properly."""
    ...
    collected_lock = threading.Lock()
    visited_start_keys = set()
    seen_key_hashes = set()  # Use hashes for deduplication
    ...
    def safe_query_map_v2(substrate_conn, start_key, max_retries=3):
        """Safely perform query_map with connection recovery on metadata errors."""
        current_conn = substrate_conn
    ...
    def worker():
        ...
        try:
            ...
            while True:
                ...
                try:
                    page_t0 = time.time()
                    qmr, local_substrate = safe_query_map_v2(local_substrate, start_key)
                    page_elapsed = time.time() - page_t0
                    
                    logging.info(
                        f"Block {block_number}: {threading.current_thread().name} "
                        f"fetched page with {len(qmr.records)} records in {page_elapsed:.2f}s"
                    )
                    ...
                    with collected_lock:
                        for rec in page_records:
                            key_hash = get_key_hash(rec[0])
                            if key_hash not in seen_key_hashes:
                                seen_key_hashes.add(key_hash)
                                # Preserve original record format
                                collected_records.append(rec)
                        ...
                        with progress_lock:
                            ...
                            if pages_done % 5 == 0:
                                logging.info(
                                    f"Block {block_number}: {pages_done} pages fetched "
                                    f"(≈{len(seen_key_hashes)} unique records)"
                                )
                    
                    # Handle pagination
                    if len(page_records) == page_size and qmr.last_key:
                        last_key_hash = get_key_hash(qmr.last_key)
                        if last_key_hash not in visited_start_keys:
                            visited_start_keys.add(last_key_hash)
                            # Use the original last_key format for next query
    ...
    logging.info(
        f"Block {block_number}: parallel StakingHotkeys fetch finished, "
        f"{len(seen_key_hashes)} unique records in {overall_elapsed:.2f}s using {fetch_workers} workers"
    )
