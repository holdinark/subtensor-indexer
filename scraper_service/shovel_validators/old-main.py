...
def fetch_all_validators_data(substrate, block_hash: str, block_timestamp, block_number: int, subnet_filter: int = None, limit_child_keys: int = None):
    """Fetch all validators data for a given block.
    
    Args:
        substrate: Substrate client instance
        block_hash: Block hash
        block_timestamp: Block timestamp
        block_number: Block number
        subnet_filter: If specified, only fetch data for this subnet
        limit_child_keys: If specified, limit the number of child key validators
    ...
    if subnet_filter is not None:
        subnet_uids = [subnet_filter] if subnet_filter in subnet_uids else []
    logging.info(f"Found {len(subnet_uids)} subnets to process")

    # Find child key validators
    child_key_validators = []
    child_key_limit = limit_child_keys if limit_child_keys is not None else 20
        ...
        if validator_info and any(validator_info.get('validator_permits', [])):
            for net_uid in subnet_uids:
                ...
                for child_hotkey, child_take in child_keys:
                    ...
                    if len(child_key_validators) >= child_key_limit:
                        break
                if len(child_key_validators) >= child_key_limit:
                    break
        if len(child_key_validators) >= child_key_limit:
    ...
    for idx, (validator_address, parent_address) in enumerate(all_validators_to_process, 1):
        try:
            ...
            validator_data = {
                ...
                "owner": info['owner'],
                "url": info['url'],
                "nominators": stats["nominators"],
                "daily_return": stats["daily_return"],
                "registrations": stats['registrations'],
                "validator_permits": stats['validator_permits'],
                "subnet_hotkey_alpha": stats['subnet_hotkey_alpha'],
                "parent_address": parent_address,
                "is_child_key": parent_address is not None
            }
...
class ValidatorsShovel(ShovelBaseClass):
    ...
    def process_block(self, n):
        ...
        try:
            ...
            validators_data = fetch_all_validators_data(substrate, block_hash, block_timestamp, n)
            ...
            logging.info(f"- Direct validators: {regular_validators}")
            logging.info(f"- Child key validators: {child_key_validators}")
            logging.info(f"- Successful inserts: {successful_inserts}")
            logging.info(f"- Failed inserts: {len(validators_data) - successful_inserts}")
